                                    ...
                                    <tbody>
                                        @foreach ($meat_renewal_list as $key => $value)
                                            <tr>
                                                ...
                                                @if($value->re_final_approve == '2')
                								<td>{{ $value->re_final_reason_for_rejection }}</td>
                								@endif
            									<td>
                                                    <a href='{{ url("/final_approve_view_renewal/{$value->id}/{$value->re_final_approve}") }}' class="btn btn-info btn-sm text-light">
                                                        <i class="zmdi zmdi-eye"></i>
                                                    </a>
                                                </td>
                                                 <?php if($value->re_final_approve == 1) { ?>
                                                 <td>
                                                     
                                                    <a href='{{ url("/generate_english_cold_renewal_pdf/{$value->id}/{$value->re_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
                                                        <i class="zmdi zmdi-file"></i> English
                                                    </a>
                                                    &nbsp;&nbsp;
                                                    <a href='{{ url("/generate_marathi_cold_renewal_pdf/{$value->id}/{$value->re_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
